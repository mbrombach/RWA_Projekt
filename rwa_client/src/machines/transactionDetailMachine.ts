...
export const transactionDetailMachine = dataMachine("transactionData").withConfig({
  services: {
    fetchData: async (ctx, event: any) => {
      ...
      const resp = await httpClient.get(`${backendUrl}/transactions/${transactionId}`);
      // @ts-ignore
    ...
    createData: async (ctx, event: any) => {
      ...
      const resp = await httpClient.post(
        `${backendUrl}/${route}/${payload.transactionId}`,
        payload
      );
      return resp.data;
    },
    updateData: async (ctx, event: any) => {
      ...
      const resp = await httpClient.patch(`${backendUrl}/transactions/${transactionId}`, payload);
      return resp.data;
    },
  },
});
